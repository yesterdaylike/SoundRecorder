...
implements Recorder.OnStateChangedListener {
	...
	static final String PLAYING_DATA_KEY = "playing_data";
	...
	static final int BITRATE_AAC = 65536;
	...
	private String mPlayingData;
	VUMeter mVUMeter;
	private BroadcastReceiver mSDCardMountEventReceiver = null;
	private BroadcastReceiver mPowerOffEventReceiver = null;
	...
	public void onCreate(Bundle icycle) {
		...
		if(mPowerOffEventReceiver == null){
			mPowerOffEventReceiver = new BroadcastReceiver() {
				@Override
		...
		registerReceiver(mPowerOffEventReceiver, iFilter);
	}
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		recorderState.putString(PLAYING_DATA_KEY, adapter.getPlayingData());
	...
	private void initResourceRefs() {
		...
		mTimerView = (TimerTextView) findViewById(R.id.timerView);
		...
		mRecordList.setDividerHeight(0);

		adapter = new RecorderAdapter(this,
				R.layout.listview_item, getPlaylist(),
				new String[] {MediaStore.Audio.Media.DISPLAY_NAME, MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.DURATION},
				new int[] { R.id._display_name,R.id.title });
		adapter.setProgressBar(mStateProgressBar);
		adapter.setPlayingData(mPlayingData);
		...
		mRecordList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if( Recorder.IDLE_STATE != mRecorder.state() ){
					onClickAllRecordButton(null);
				}
				
				RelativeLayout layout = (RelativeLayout) mStateProgressBar.getParent();
				if(null!=layout){
				...
				mPlayingData = view.getTag().toString();
				adapter.setPlayingData(mPlayingData);
				
				adapter.notifyDataSetChanged();
				
				mRecorder.startPlayback(view.getTag().toString(), view.getTag(R.id.MEDIASTORE_AUDIO_MEDIA_DURATION));
			}
		});
	}
	...
	public void onClickAllRecordButton(View button){
	...
	public void onDestroy() {
		...
		if(mPowerOffEventReceiver != null){
			unregisterReceiver(mPowerOffEventReceiver);
			mPowerOffEventReceiver = null;
		}
		super.onDestroy();
	}
	...
	private Cursor getPlaylist() {
		...
		Cursor cursor = query(uri, ids, null, null, MediaStore.Audio.Playlists._ID+" desc");
		return cursor;
	}
	...
	private void updateUi() {
		...
		case Recorder.IDLE_STATE:
			...
			adapter.setPlayingData(null);
			mPlayingData = null;
			mStateMessage2.setText(null);
			if (mRecorder.sampleLength() == 0) {
		...
		case Recorder.PLAYING_STATE: 
			mStateMessage2.setText(res.getString(R.string.playing));
			//mExitButtons.setVisibility(View.VISIBLE);
			//mVUMeter.setVisibility(View.INVISIBLE);
